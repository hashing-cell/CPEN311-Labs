...
volatile int toCopy = 0x12;

volatile int src_arr[9] = {
  0x11, 0x22, 0x33, 
  0x44, 0x55, 0x66, 
  0x77, 0x88, 0x99};


volatile int dest_arr[9] = {
  0xDEADBEEF, 0xDEADBEEF, 0xDEADBEEF, 
  0xDEADBEEF, 0xDEADBEEF, 0xDEADBEEF, 
  0xDEADBEEF, 0xDEADBEEF, 0xDEADBEEF};

// after loading test_00.bin into 0x0880 0000
volatile      int *dram_src        = (volatile      int *) 0x08800268;
volatile      int *dram_dest       = (volatile      int *) 0x08800260;
...
int main()
{
    *hex = 0x3f; /* display - */
    ...
    // pio test
    // wordcopy((volatile int *)hex, &toCopy, 1);

    // // on chip memory test
    // wordcopy(dest_arr, src_arr, 9);
    
    // Dram test
    wordcopy(dram_dest, dram_src, 2);
